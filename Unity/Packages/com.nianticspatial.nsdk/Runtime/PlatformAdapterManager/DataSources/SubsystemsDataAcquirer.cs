...
namespace NianticSpatial.NSDK.AR.PAM
{
    ...
    internal class SubsystemsDataAcquirer : PlatformDataAcquirer
    {
        ...
        private bool _usingNsdkOcclusion;
        ...
        public override void Dispose()
        {
            ...
            if (_autoEnabledLocationServices && Input.location.status == LocationServiceStatus.Running)
            {
                Log.Info
                (
                    "NSDK enabled location services because they were required by an NSDK feature. " +
                    "NSDK is now shutting down. If location services are no longer required, they must be " +
                    "separately disabled."
                );
            }
            ...
            if (_autoEnabledCompass && Input.compass.enabled)
            {
                Log.Info
                (
                    "NSDK enabled the compass because it was required by an NSDK feature. " +
                    "NSDK is now shutting down. If the compass data is no longer required, it must be " +
                    "separately disabled."
                );
            }
        ...
        protected virtual void OnSubsystemsLoaded(XRLoader loader)
        {
            ...
            _usingNsdkOcclusion = _occlusionSubsystem is NsdkOcclusionSubsystem;
        }

        // See NsdkARCoreLoader.UpgradeCameraConfigurationIfNeeded Note #2 for why we need this logic
        private void OnSessionStateChanged(ARSessionStateChangedEventArgs args)
        {
            ...
            {
                Log.Info("ARSession was reset. NSDK functionality will be limited until tracking is re-established.");
        ...
        public override bool TryGetCameraTimestampMs(out double timestampMs)
        {
            ...
            if (_cameraSubsystem.TryGetLatestFrame(emptyParams, out var frame))
            {
                ...
                {
                    timestampMs = (double)timestampNs / 1_000_000;
                    return true;
                }
            }
        ...
        public override bool TryGetCpuImage(out NsdkCpuImage cpuImage)
        {
            ...
            return _cameraSubsystem.TryAcquireLatestCpuImage(out _cameraCpuImage) &&
                NsdkCpuImage.TryGetFromXRCpuImage(_cameraCpuImage, out cpuImage);
        }

        public override bool TryGetDepthCpuImage
        (
            out NsdkCpuImage depthCpuImage,
        ...
        )
        {
            ...
            if (hasDepthImage)
            {
                hasDepthImage = NsdkCpuImage.TryGetFromXRCpuImage(_depthCpuImage, out depthCpuImage);
                if (hasDepthImage &&
                    _occlusionSubsystem.TryAcquireEnvironmentDepthConfidenceCpuImage(out _depthConfidenceCpuImage))
                {
                    NsdkCpuImage.TryGetFromXRCpuImage(_depthConfidenceCpuImage, out confidenceCpuImage);
                }
            }
        ...
        public override bool TryGetCompass(out CompassDataCStruct compass)
        {
            ...
            // since NSDK 2.x, so we will continue to do that, even on Android, for continuity's sake.
        ...
        protected void StartCompassIfNeeded()
        {
            ...
            Log.Info("The device's compass is required by an enabled NSDK feature, so it is being turned on.");
        ...
        protected void TryStartLocation()
        {
            ...
            if (Input.location.status == LocationServiceStatus.Stopped ||
                Input.location.status == LocationServiceStatus.Failed)
            {
                Log.Info("Location services are required by an enabled NSDK feature, so NSDK will attempt to enable them.");
        ...
        private void MissingLocationPermissionLog()
        {
            ...
            (
                ...
                "Some NSDK features are limited without location data."
            );
        }
