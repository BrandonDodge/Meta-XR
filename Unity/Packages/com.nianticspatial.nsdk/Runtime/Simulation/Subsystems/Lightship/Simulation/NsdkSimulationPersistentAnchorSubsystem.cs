...
{
    public class NsdkSimulationPersistentAnchorSubsystem :
        XRPersistentAnchorSubsystem
    {
        private const string SubsystemId = "Nsdk-Simulation-PersistentAnchor";
        ...
        static void RegisterDescriptor()
        {
            var cinfo = new XRPersistentAnchorSubsystemDescriptor.Cinfo
            {
                ...
                subsystemTypeOverride = typeof(NsdkSimulationPersistentAnchorSubsystem),
        ...
        private class NsdkSimulationProvider : Provider
        {
            ...
            private readonly NsdkSimulationPersistentAnchorParams _persistentAnchorParams;
            ...
            {
                Log.Debug($"{nameof(NsdkSimulationPersistentAnchorSubsystem)}.{nameof(NsdkSimulationProvider)} ctor");

                _sessionId = Guid.NewGuid();
                _persistentAnchorParams =
                    NsdkSettingsHelper.ActiveSettings.NsdkSimulationParams.SimulationPersistentAnchorParams;
            }
            ...
            )
            {
                if (!_started)
                {
                    Log.Warning("NsdkSimulationProvider not started. Cannot localize anchor.");
                    anchor = default;
                    return false;
                }
