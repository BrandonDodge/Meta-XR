...
namespace NianticSpatial.NSDK.AR.NavigationMesh
{
    ...
    /// </summary>
    [PublicAPI("apiref/Niantic/Lightship/AR/NavigationMesh/NsdkNavMeshAgent/")]
    public class NsdkNavMeshAgent : MonoBehaviour
    {
        ...
        private NsdkNavMeshManager _nsdkNavMeshManager;
        ...
        private void Start()
        {
            ...
            //find the NsdkNavMesh manager and connect it.
            _nsdkNavMeshManager = GameObject.FindAnyObjectByType<NsdkNavMeshManager>();

            //fail if missing
            if (_nsdkNavMeshManager == null)
                throw new ArgumentException("You need to add a LightshipNavMeshManager to the scene");

            _nsdkNavMesh = _nsdkNavMeshManager.NsdkNavMesh;
        }
        ...
        public void SetDestination(Vector3 destination)
        {
            ...
            if (_nsdkNavMesh == null)
                return;

            _destination = destination;

            Vector3 startOnBoard;
            _nsdkNavMesh.FindNearestFreePosition(transform.position, out startOnBoard);

            bool result = _nsdkNavMesh.CalculatePath(startOnBoard, destination, _agentConfig, out _path);
        ...
        {
            if (_nsdkNavMesh == null || _nsdkNavMesh.Area == 0)
                return;

            if (_nsdkNavMesh.IsOnNavMesh(transform.position, 0.2f))
                return;

            List<Waypoint> pathToNavMesh = new List<Waypoint>();
            Vector3 nearestPosition;
            _nsdkNavMesh.FindNearestFreePosition(transform.position, out nearestPosition);
            ...
            (
                ...
                Utils.PositionToTile(transform.position, _nsdkNavMesh.Settings.TileSize)
            ));
            ...
            (
                ...
                Utils.PositionToTile(nearestPosition, _nsdkNavMesh.Settings.TileSize)
            ));
        ...
        private IEnumerator Move(Transform actor, IList<Waypoint> path)
        {
            ...
            while (destIdx < path.Count)
            {
                ...
                //NsdkNavMesh is an average height so can be under/over the mesh
                ...
                if (Physics.Raycast(from, dir, out hit, 100, _nsdkNavMesh.Settings.LayerMask))
                {
