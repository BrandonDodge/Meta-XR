...
{
    /// <summary>
    /// The NSDK implementation of the <c>XRScanningSubsystem</c>. Do not create this directly.
    /// Use the <c>SubsystemManager</c> instead.
    /// </summary>
    [Preserve]
    public sealed class NsdkScanningSubsystem : XRScanningSubsystem, ISubsystemWithMutableApi<IApi>
    {
        internal class NsdkProvider : Provider
        {
            ...
            public override int GetFrameCount()
            {
                ...
                return _api.GetFrameCount(_nativeProviderHandle);
            }
            ...
            public override void SaveCurrentScan()
            {
                ...
                if (_api.GetFrameCount(_nativeProviderHandle) <= 0)
                {
                ...
                set
                {
                    ...
                    if (value.RaycasterVisualizationEnabled || value.VoxelVisualizationEnabled)
                    {
                        if (!value.UseEstimatedDepth)
                        {
                            ...
                            // code.
                            bool lidarEnabled = NsdkSettingsHelper.ActiveSettings.PreferLidarIfAvailable;
                            if (!lidarEnabled)
                            {
                                Log.Warning("Depth data is required for scanning visualization, but NSDK's " +
            ...
            )
            {
                ...
                raycastBufferDescriptor = XRTextureDescriptorUtils.Create(
                ...
                raycastNormalBufferDescriptor = XRTextureDescriptorUtils.Create(
                ...
                raycastPositionAndConfidenceDescriptor = XRTextureDescriptorUtils.Create(
        ...
        private static void RegisterDescriptor()
        {
            var cinfo = new XRScanningSubsystemDescriptor.Cinfo
            {
                ...
                subsystemTypeOverride = typeof(NsdkScanningSubsystem),
            };
