...
namespace NianticSpatial.NSDK.AR.Subsystems.Semantics
{
    internal class MockApi : IApi
    {
        ...
        private XRCpuImage.Api _cpuImageApi => NsdkCpuImageApi.Instance;
        ...
        public void Configure(IntPtr nativeProviderHandle, UInt32 framesPerSecond, UInt32 numThresholds, IntPtr thresholds, HashSet<SemanticsChannel> suppressionMaskChannels) {
            InitializeSuppressionMaskBuffer(GetFlags(suppressionMaskChannels));
		}
        ...
        )
        {
            ...
            unsafe
            {
                ...
                semanticsChannelDescriptor = XRTextureDescriptorUtils.Create(
        ...
        )
        {
            ...
            unsafe
            {
                ...
                cpuImage = default; // new NsdkpCpuBuffer(nativeProviderHandle, nativePtr, new Vector2Int(_width, _height), NsdkCpuBuffer.Format.DepthFloat32);
            }
        ...
        )
        {
            ...
            unsafe
            {
                ...
                packedSemanticsDescriptor = XRTextureDescriptorUtils.Create(
                    tex.GetNativeTexturePtr(),
                    _width,
                    _height,
                    0,
        ...
        {
            var semanticChannels = new List<SemanticsChannel>(AllChannels);

            if (!_initializedPackedBuffer)
                InitializePackedBuffer(semanticChannels);

            return semanticChannels;
        }
            ...
            out Matrix4x4 samplerMatrix)
        {
            ...
            unsafe
            {
                ...
                suppressionMaskDescriptor = XRTextureDescriptorUtils.Create(
                    tex.GetNativeTexturePtr(),
                    _width,
                    _height,
                    0,
        ...
        private void InitializePackedBuffer(List<SemanticsChannel> semanticChannels)
        {
            Debug.Assert(default != semanticChannels && semanticChannels.Count > 0,
                "For the sake of the mock, it's expected that channels are available at the time that InitializePackedBuffer is called");
            int allChannelsBitmask = 0;

            // Claim that all channels are detected for simplicity
            for (int channel = 0; channel < semanticChannels.Count; channel++)
            {
